...
// Fetch all-time favorite perfumes from the database with their LOWEST price
$perfumes = [];
$query = "SELECT 
             p.*,
             COALESCE(
...
// Debug output
if ($debug_mode) {
    echo "<pre>DEBUG: Database Structure\n";
    // Check tables
    $tables = $conn->query("SHOW TABLES")->fetch_all(MYSQLI_ASSOC);
    print_r($tables);

    // Check perfume_sizes data
    $sizes = $conn->query("SELECT * FROM perfume_sizes LIMIT 10")->fetch_all(MYSQLI_ASSOC);
    echo "Perfume Sizes Sample:\n";
    print_r($sizes);

    // Check the actual query results
    echo "Final Perfume Data:\n";
    print_r($perfumes);
    echo "</pre>";
}
?>
